...
export const ScannerView: React.FC = () => {
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      let extracted = extractDependencies(code, activeLang);

      // Smart auto-detection fallback: if no dependencies detected in selected language,
      // check the alternate language in case the user pasted JS into Python mode or vice versa
      if (extracted.length === 0) {
        const alternateLang: SupportedLanguage = activeLang === 'python' ? 'javascript' : 'python';
        const alternateExtracted = extractDependencies(code, alternateLang);
        if (alternateExtracted.length > 0) {
          activeLang = alternateLang;
          extracted = alternateExtracted;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-6">
        <div>
          <div className="flex items-center space-x-2">
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Code Dependency Scanner</h1>
            <span className="px-2.5 py-0.5 rounded-full text-2xs font-bold bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800">
              Part 3 Final
            </span>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            ...
            className={`flex items-center space-x-2 px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all border ${
              language === 'python'
                ? 'bg-white dark:bg-slate-800 text-indigo-700 dark:text-indigo-300 shadow-xs border-indigo-200 dark:border-indigo-700'
                : 'bg-slate-100 dark:bg-slate-800/60 text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white border-slate-200 dark:border-slate-700'
            }`}
          >
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xs overflow-hidden">
        {/* Editor Toolbar */}
        <div className="px-4 py-3 bg-slate-50 dark:bg-slate-900/90 border-b border-slate-200 dark:border-slate-800 flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center space-x-3 text-xs text-slate-700 dark:text-slate-300 font-medium">
            <span className="inline-flex items-center space-x-1.5 font-mono text-slate-700 dark:text-slate-300">
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className="inline-flex items-center space-x-1 px-3 py-1.5 rounded-lg text-xs font-medium text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200/60 dark:hover:bg-slate-800 transition-colors"
        ...
        <div className="px-4 py-2.5 bg-slate-50 dark:bg-slate-900/90 border-t border-slate-200 dark:border-slate-800 flex flex-wrap items-center justify-between gap-2 text-xs text-slate-600 dark:text-slate-400">
          <div className="flex items-center space-x-2">
          ...
          <div className="flex items-center space-x-3 text-slate-600 dark:text-slate-400 font-medium">
            ...
            <span>•</span>
            <span className="text-emerald-700 dark:text-emerald-400 font-semibold">Live Real-time Endpoints</span>
          </div>
        </div>
      </div>

      {/* Normalization Reference Banner */}
      <div className="p-3.5 rounded-xl bg-blue-50/70 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-900 flex flex-col md:flex-row md:items-center justify-between gap-3 text-xs text-blue-900 dark:text-blue-200">
        <div className="flex items-center space-x-2">
          ...
          <span>
            ...
            <span className="font-mono ml-2 font-medium bg-white dark:bg-slate-900 px-2 py-0.5 rounded-md border border-blue-200 dark:border-blue-800">PIL → Pillow</span>
            <span className="font-mono ml-1.5 font-medium bg-white dark:bg-slate-900 px-2 py-0.5 rounded-md border border-blue-200 dark:border-blue-800">cv2 → opencv-python</span>
            <span className="font-mono ml-1.5 font-medium bg-white dark:bg-slate-900 px-2 py-0.5 rounded-md border border-blue-200 dark:border-blue-800">sklearn → scikit-learn</span>
            <span className="font-mono ml-1.5 font-medium bg-white dark:bg-slate-900 px-2 py-0.5 rounded-md border border-blue-200 dark:border-blue-800">bs4 → beautifulsoup4</span>
          </span>
        </div>
      </div>
      ...
      {isScanning && (
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-indigo-200 dark:border-indigo-800 p-6 shadow-xs flex items-center space-x-4 animate-pulse">
          <div className="w-10 h-10 rounded-xl bg-indigo-50 dark:bg-indigo-950/60 text-indigo-600 dark:text-indigo-400 flex items-center justify-center shrink-0">
            <Loader2 className="w-6 h-6 animate-spin text-indigo-600 dark:text-indigo-400" />
          </div>
          <div>
            <h4 className="text-sm font-bold text-slate-900 dark:text-white">Live Upstream Verification In Progress</h4>
            <p className="text-xs text-indigo-700 dark:text-indigo-300 mt-0.5 font-medium">{scanMessage || 'Contacting public registries...'}</p>
          </div>
        </div>
      )}
      ...
      {/* Scan Error Banner */}
      {scanError && (
        <div className="p-4 bg-rose-50 dark:bg-rose-950/40 rounded-xl border border-rose-200 dark:border-rose-900 text-xs text-rose-800 dark:text-rose-300 flex items-center space-x-3">
      ...
      {!hasScanned && !isScanning ? (
        ...
        <div id="scanner-empty-state" className="bg-white dark:bg-slate-900 rounded-xl border border-dashed border-slate-300 dark:border-slate-800 p-10 sm:p-12 text-center shadow-xs">
          <div className="w-14 h-14 rounded-2xl bg-indigo-50 dark:bg-indigo-950/60 text-indigo-600 dark:text-indigo-400 flex items-center justify-center mx-auto mb-4 border border-indigo-100 dark:border-indigo-800">
            <Code2 className="w-7 h-7" />
          </div>
          <h3 className="text-base font-bold text-slate-900 dark:text-white">Ready to Inspect Dependencies</h3>
          <p className="text-sm text-slate-500 dark:text-slate-400 max-w-md mx-auto mt-1">
            Click <strong className="text-slate-700 dark:text-slate-200 font-semibold">“Scan & Verify”</strong> or load one of the test demos to inspect package registry records and run AI threat intelligence.
          </p>
          <div className="mt-5 flex flex-wrap justify-center gap-3">
            <button
              onClick={handleLoadPythonDemo}
              className="inline-flex items-center space-x-1.5 px-4 py-2 rounded-lg text-xs font-semibold text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-950/50 hover:bg-indigo-100 dark:hover:bg-indigo-900/60 border border-indigo-200 dark:border-indigo-800 transition-colors"
            >
            ...
            <button
              onClick={handleLoadJavaScriptDemo}
              className="inline-flex items-center space-x-1.5 px-4 py-2 rounded-lg text-xs font-semibold text-amber-800 dark:text-amber-300 bg-amber-50 dark:bg-amber-950/50 hover:bg-amber-100 dark:hover:bg-amber-900/60 border border-amber-200 dark:border-amber-800 transition-colors"
            >
              <Sparkles className="w-4 h-4" />
              <span>Load JavaScript Demo</span>
            </button>
        ...
        <div id="scanner-results-section" className="space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-4 shadow-xs">
              <span className="text-2xs text-slate-500 dark:text-slate-400 uppercase tracking-wider block font-semibold">
                Total Dependencies
              </span>
              <span className="text-2xl font-bold text-slate-900 dark:text-white mt-1 block font-mono">{totalCount}</span>
              <span className="text-3xs text-slate-400 dark:text-slate-500 mt-1 block">Extracted statically</span>
            </div>

            {/* Verified */}
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-emerald-200 dark:border-emerald-800/60 p-4 shadow-xs bg-emerald-50/20 dark:bg-emerald-950/20">
              <span className="text-2xs text-emerald-700 dark:text-emerald-400 uppercase tracking-wider block font-semibold flex items-center">
                <CheckCircle2 className="w-3 h-3 mr-1" />
                Verified
              </span>
              <span className="text-2xl font-bold text-emerald-600 dark:text-emerald-400 mt-1 block font-mono">{verifiedCount}</span>
              <span className="text-3xs text-emerald-600/80 dark:text-emerald-400/70 mt-1 block">Confirmed on registry</span>
            </div>

            {/* Unverified */}
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-rose-200 dark:border-rose-800/60 p-4 shadow-xs bg-rose-50/20 dark:bg-rose-950/20">
              <span className="text-2xs text-rose-700 dark:text-rose-400 uppercase tracking-wider block font-semibold flex items-center">
                <XCircle className="w-3 h-3 mr-1" />
                Unverified
              </span>
              <span className="text-2xl font-bold text-rose-600 dark:text-rose-400 mt-1 block font-mono">{unverifiedCount}</span>
              <span className="text-3xs text-rose-600/80 dark:text-rose-400/70 mt-1 block">HTTP 404 (Hallucination)</span>
            </div>

            {/* Review Required */}
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-amber-200 dark:border-amber-800/60 p-4 shadow-xs bg-amber-50/20 dark:bg-amber-950/20">
              <span className="text-2xs text-amber-800 dark:text-amber-400 uppercase tracking-wider block font-semibold flex items-center">
                <AlertTriangle className="w-3 h-3 mr-1" />
                Review Required
              </span>
              <span className="text-2xl font-bold text-amber-600 dark:text-amber-400 mt-1 block font-mono">{reviewRequiredCount}</span>
              <span className="text-3xs text-amber-700/80 dark:text-amber-400/70 mt-1 block">Normalized / Built-in</span>
            </div>

            {/* Registry Unavailable */}
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-4 shadow-xs bg-slate-50/50 dark:bg-slate-800/40 col-span-2 sm:col-span-1">
              <span className="text-2xs text-slate-600 dark:text-slate-400 uppercase tracking-wider block font-semibold flex items-center">
                <WifiOff className="w-3 h-3 mr-1" />
                Unavailable
              </span>
              <span className="text-2xl font-bold text-slate-700 dark:text-slate-200 mt-1 block font-mono">{registryUnavailableCount}</span>
              <span className="text-3xs text-slate-500 dark:text-slate-400 mt-1 block">Network timeout</span>
            </div>
          </div>

          {/* Action Bar with Gemini Threat Analysis Trigger */}
          <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-4 sm:p-5 shadow-xs flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="space-y-1">
              <div className="flex items-center space-x-2">
                <h3 className="text-sm font-bold text-slate-900 dark:text-white">Verification Complete</h3>
                <span className="text-2xs font-semibold px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300">
            ...
            <div className="flex flex-wrap items-center gap-2.5">
              ...
              <button
                ...
                className="inline-flex items-center space-x-1.5 px-3 py-2 rounded-lg text-xs font-medium text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
          ...
          {results.length === 0 ? (
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-8 text-center text-slate-500 dark:text-slate-400">
              ...
              <p className="text-xs mt-1 text-slate-400 dark:text-slate-500">Make sure your code contains import or require statements.</p>
            </div>
          ) : (
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xs overflow-hidden">
              <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-800 flex flex-wrap items-center justify-between gap-2 bg-slate-50/50 dark:bg-slate-800/40">
                <div>
                  <h3 className="font-bold text-slate-900 dark:text-white text-sm">Dependency Verification Evidence</h3>
                ...
                <div className="flex items-center space-x-3">
                  <button
                    ...
                    className="inline-flex items-center space-x-1 text-xs font-semibold text-indigo-700 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300"
                  >
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                  <thead className="bg-slate-50 dark:bg-slate-800/70 text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase tracking-wider border-b border-slate-200 dark:border-slate-800">
                    <tr>
                        ...
                        <React.Fragment key={dep.id}>
                          <tr
                            ...
                            className={`cursor-pointer transition-colors ${
                              isExpanded ? 'bg-indigo-50/30 dark:bg-indigo-950/30' : 'hover:bg-slate-50/70 dark:hover:bg-slate-800/40'
                            }`}
                          >
                            {/* Package & Import Name */}
                            <td className="px-6 py-4 align-top">
                              <div className="space-y-1">
                                <div className="flex items-center space-x-2">
                                  ...
                                  {dep.isNormalized && (
                                    <span className="inline-flex items-center space-x-1 px-2 py-0.5 rounded-md text-2xs font-semibold bg-blue-100 dark:bg-blue-950/60 text-blue-800 dark:text-blue-300 border border-blue-200 dark:border-blue-800">
                                      <span>alias:</span>
                              ...
                              <div className="space-y-1.5">
                                ...
                                {dep.status === 'REGISTRY UNAVAILABLE' && (
                                  <button
                                    ...
                                    className="block text-2xs font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 underline"
                                  >
                            ...
                            <td className="px-6 py-4 align-top">
                              <div className="space-y-1.5 max-w-sm">
                                {dep.registryMetadata?.exists ? (
                                  <>
                                    <div className="flex flex-wrap items-center gap-2">
                                      {dep.registryMetadata.latestVersion && (
                                        <span className="inline-flex items-center px-2 py-0.5 rounded text-2xs font-mono font-bold bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700">
                                    ...
                                    {dep.registryMetadata.summary && (
                                      <p className="text-2xs text-slate-600 dark:text-slate-300 line-clamp-1">
                                        {dep.registryMetadata.summary}
                                      </p>
                                    )}
                                  </>
                                  ...
                                  <div className="space-y-1">
                                    <span className="inline-flex items-center px-2 py-0.5 rounded text-2xs font-semibold bg-rose-100 dark:bg-rose-950/60 text-rose-800 dark:text-rose-300 border border-rose-200 dark:border-rose-800">
                                      HTTP 404 Not Found
                                    </span>
                                    <p className="text-2xs text-rose-700 dark:text-rose-400 font-medium">
                            ...
                            <td className="px-6 py-4 font-mono text-xs align-top">
                              <div className="bg-slate-100 dark:bg-slate-800 rounded-md px-2.5 py-1.5 border border-slate-200 dark:border-slate-700 flex items-center justify-between group max-w-xs">
                                <span className="text-slate-700 dark:text-slate-300 truncate">
                                  <span className="text-indigo-600 dark:text-indigo-400 font-semibold mr-1.5">
                                    L{dep.evidence.lineNumber}:
                                  </span>
                                  <span className="text-slate-900 dark:text-slate-100">{dep.evidence.rawLine.trim()}</span>
                                </span>
                              </div>
                            </td>
                          ...
                          {isExpanded && (
                            <tr className="bg-slate-50/80 dark:bg-slate-950/80 border-b border-slate-200 dark:border-slate-800">
                                ...
                                <div className="p-4 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xs space-y-4">
                                    ...
                                    <div className="flex items-center space-x-2">
                                      {dep.registryMetadata?.registryUrl && (
                                        <a
                                          ...
                                          className="inline-flex items-center space-x-1 text-xs font-semibold text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300"
                                        >
                                      ...
                                      {dep.registryMetadata?.projectUrl &&
                                        dep.registryMetadata.projectUrl !== dep.registryMetadata.registryUrl && (
                                          <a
                                            ...
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="inline-flex items-center space-x-1 text-xs text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 ml-2"
                                          >
                                  ...
                                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
                                    ...
                                    <div className="space-y-2">
                                      <span className="text-3xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider block">
                                        Source Syntax Evidence
                                      </span>
                                      <div className="p-3 rounded-lg bg-slate-900 dark:bg-slate-950 text-slate-100 font-mono text-xs space-y-1 border border-slate-800">
                                      ...
                                      {dep.isNormalized && (
                                        <div className="text-2xs text-blue-900 dark:text-blue-300 bg-blue-50 dark:bg-blue-950/40 p-2.5 rounded-lg border border-blue-200 dark:border-blue-800">
                                    ...
                                    <div className="space-y-2">
                                      ...
                                      <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 space-y-1.5 text-2xs">
                                        <div className="flex justify-between">
                                          <span className="text-slate-500 dark:text-slate-400">HTTP Status:</span>
                                          <span className="font-mono font-bold text-slate-900 dark:text-white">
                                            {dep.registryMetadata?.exists ? '200 OK (Found)' : dep.status === 'UNVERIFIED' ? '404 Not Found' : 'Unconfirmed'}
                                          </span>
                                        </div>
                                        <div className="flex justify-between">
                                          <span className="text-slate-500 dark:text-slate-400">Latest Release:</span>
                                          <span className="font-mono font-bold text-slate-900 dark:text-white">
                                        ...
                                        {dep.registryMetadata?.author && (
                                          <div className="flex justify-between">
                                            <span className="text-slate-500 dark:text-slate-400">Author / Maintainer:</span>
